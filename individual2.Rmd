...
For my two analyses, I will look at how the origin of the cocoa bean affects the result of the rating, and I will look at which company has the highest average ratings.

```{r}
beanRatings <- choco[, c("Bean_Origin", "Rating")]
aggregatedTable <- aggregate(x=beanRatings$Rating,by=list(beanRatings$Bean_Origin), FUN=mean)
beanCounts <- data.frame(table(choco[, c("Bean_Origin")]))
aggregatedTable$Freq <- beanCounts$Freq
x <- c("Bean_Origin", "Avg_Rating", "Freq")
colnames(aggregatedTable) <- x
kable(aggregatedTable[with(aggregatedTable, order(-Avg_Rating)),])
```
  
To determine which countries of origin had the best ratings, the average rating for each bean origin was put into the table above. It can be seen that Tobago has the highest average rating (by far) and Matinique, Principe, Puerto Rico and St. Vincent-Grenadines are all tied for worst average rating. However, Since the top 2 countries only have 2 and 1 entry, respectively, it may be unfair to claim those are the top countries. Looking at the third entry, Solomon Islands, this country has 10 entries. Depending on how one measures the "best", the results can vary slightly.
```{r}
ggplot(aggregatedTable, aes(x=Bean_Origin, y=Avg_Rating)) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
  
The above plot shows graphically all the bean origins and hwo they compare with one another.

```{r}
companyRatings <- choco[, c("Company", "Rating")]
aggregatedTable <- aggregate(x=companyRatings$Rating,by=list(companyRatings$Company), FUN=mean)
companyCounts <- data.frame(table(choco$Company))
aggregatedTable$Freq <- companyCounts$Freq
x <- c("Company", "Avg_Rating", "Freq")
colnames(aggregatedTable) <- x
kable(aggregatedTable[with(aggregatedTable, order(-Avg_Rating)),])
```
  
Looking at the table above, it can be seen which are the top companies and which are the worst, in terms of ratings. However, similar to the previous analysis, it can be seen that the top 5 countries have limited entries in the dataset, so it may be more fair to claim Idilio is the best company on average, given its amount of enties and still high average rating.

```{r}
top50 <- head(aggregatedTable[with(aggregatedTable, order(-Avg_Rating)),], 50)
ggplot(top50, aes(x=Company, y=Avg_Rating)) + geom_bar(stat="identity") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
  
Finally, the graph above visually compares the top 50 companies.